    ...
    fn fetch_metadata_(
            ...
            .is_some_and(|v| *v >= MINIMUM_TOOLCHAIN_VERSION_SUPPORTING_LOCKFILE_PATH)
        {
            ...
            if using_lockfile_copy {
                if config.toolchain_version.as_ref().is_some_and(|v| {
                    *v >= MINIMUM_TOOLCHAIN_VERSION_USING_LOCKFILE_ENV
                }) {
                    other_options.push("-Zlockfile-path".to_owned());
                    meta.env(
                        "CARGO_RESOLVER_LOCKFILE_PATH",
                        target_lockfile.as_str(),
                    );
                } else {
        ...
        if using_lockfile_copy {
            ...
            meta.env("__CARGO_TEST_CHANNEL_OVERRIDE_DO_NOT_USE_THIS", "nightly");
        }
