...
impl CrateCache {
    ...
    pub async fn download_crate(&self, name: &str, version: &str) -> Result<PathBuf> {
        ...
        let response = self.client
            .get(&url)
            .header("User-Agent", "mcp-rust-docs/0.1.0")
            .send()
            .await
    ...
    pub async fn generate_docs(&self, name: &str, version: &str) -> Result<PathBuf> {
        ...
        if !source_path.exists() {
            bail!("Source not found for {}-{}. Download it first.", name, version);
        }
        ...
        std::fs::copy(&json_file, &docs_path)
            .context("Failed to copy documentation to cache")?;
        
        tracing::info!("Successfully generated documentation for {}-{}", name, version);
        Ok(docs_path)
    }
    ...
    pub async fn load_docs(&self, name: &str, version: &str) -> Result<rustdoc_types::Crate> {
        ...
        let crate_docs: rustdoc_types::Crate = serde_json::from_str(&json_string)
